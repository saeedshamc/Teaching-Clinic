...
**جلسهٔ 39 — معماری CSS با BEM | ۹۰ دقیقه**

### پیش‌نیازها

- مفاهیم جلسهٔ 38: دسترسی‌پذیری وب
...
1. مفهوم «معماری CSS با BEM» را با یک مثال و واژگان فارسی/English توضیح دهید.
...
موضوع **معماری CSS با BEM** یک جزیرهٔ جدا نیست؛ مستقیماً روی کیفیت، نگهداری و تحویل رابط در چرخهٔ واقعی توسعه اثر می‌گذارد. یادگیری آن به‌صورت تصمیم‌محور باعث می‌شود به‌جای حفظ کردن تگ یا property، بتوانید برای محتوای واقعی راه‌حل درست انتخاب کنید و دلیل انتخابتان را توضیح دهید.

### مفهوم عمیق: از کد تا نتیجه

مرورگر کد را از بالا به پایین parse می‌کند، آن را به ساختارهای داخلی مانند DOM و CSSOM تبدیل می‌کند و سپس style، layout و paint را انجام می‌دهد. هنگام کار با **معماری CSS با BEM** همیشه سه پرسش داشته باشید: «معنای این کد چیست؟»، «مرورگر چگونه آن را محاسبه می‌کند؟» و «کاربر با keyboard، موبایل یا صفحه‌خوان چه تجربه‌ای دارد؟». این مدل ذهنی از آزمون‌وخطای تصادفی جلوگیری می‌کند.

### مرز component در BEM

در `.card__title--featured` بخش‌ها روشن‌اند: block مستقل، element وابسته و modifier برای تغییر حالت. modifier نباید به‌تنهایی استفاده شود و nesting عمیق selectorها مزیت BEM را از بین می‌برد.
...
فایل [`examples/01_bem_card.html`](./examples/01_bem_card.html) را مبنا قرار دهید:

1. فایل را در مرورگر باز کنید و بدون دیدن کد، اجزای صفحه را نام ببرید.
2. سورس را بخوانید و هر بخش مرتبط با «معماری CSS با BEM» را با یک کامنت مشخص کنید.
...
مثال اصلی را از حافظه در یک فایل تازه بازسازی کنید و سه تصمیم خود دربارهٔ «معماری CSS با BEM» را در کامنت‌ها توضیح دهید. سپس یک screenshot یا یادداشت از بررسی DevTools ثبت کنید.
...
اکنون باید بتوانید **معماری CSS با BEM** را توضیح دهید، در نمونه تشخیص دهید، به‌صورت مرحله‌ای پیاده کنید و با ابزار مرورگر بسنجید. گام بعدی **جلسهٔ 40: Reset و Normalize** است؛ پیش از رفتن، تمرین چالشی را نگه دارید تا در جلسهٔ بعد روی همان خروجی توسعه دهید.
