...
**جلسهٔ 47 — اشکال‌زدایی با DevTools | ۹۰ دقیقه**

### پیش‌نیازها

- مفاهیم جلسهٔ 46: مبانی Sass
...
1. مفهوم «اشکال‌زدایی با DevTools» را با یک مثال و واژگان فارسی/English توضیح دهید.
...
موضوع **اشکال‌زدایی با DevTools** یک جزیرهٔ جدا نیست؛ مستقیماً روی کیفیت، نگهداری و تحویل رابط در چرخهٔ واقعی توسعه اثر می‌گذارد. یادگیری آن به‌صورت تصمیم‌محور باعث می‌شود به‌جای حفظ کردن تگ یا property، بتوانید برای محتوای واقعی راه‌حل درست انتخاب کنید و دلیل انتخابتان را توضیح دهید.

### مفهوم عمیق: از کد تا نتیجه

مرورگر کد را از بالا به پایین parse می‌کند، آن را به ساختارهای داخلی مانند DOM و CSSOM تبدیل می‌کند و سپس style، layout و paint را انجام می‌دهد. هنگام کار با **اشکال‌زدایی با DevTools** همیشه سه پرسش داشته باشید: «معنای این کد چیست؟»، «مرورگر چگونه آن را محاسبه می‌کند؟» و «کاربر با keyboard، موبایل یا صفحه‌خوان چه تجربه‌ای دارد؟». این مدل ذهنی از آزمون‌وخطای تصادفی جلوگیری می‌کند.

### گردش‌کار DevTools

عنصر خراب را Inspect کنید، در **Styles** قانون خط‌خورده و در **Computed** مقدار نهایی را ببینید، Box Model و Grid/Flex overlay را فعال کنید، اصلاح را موقت آزمایش و سپس همان اصلاح را در فایل منبع ثبت کنید.
...
فایل [`examples/01_intentional_typo.html`](./examples/01_intentional_typo.html) را مبنا قرار دهید:

1. فایل را در مرورگر باز کنید و بدون دیدن کد، اجزای صفحه را نام ببرید.
2. سورس را بخوانید و هر بخش مرتبط با «اشکال‌زدایی با DevTools» را با یک کامنت مشخص کنید.
...
مثال اصلی را از حافظه در یک فایل تازه بازسازی کنید و سه تصمیم خود دربارهٔ «اشکال‌زدایی با DevTools» را در کامنت‌ها توضیح دهید. سپس یک screenshot یا یادداشت از بررسی DevTools ثبت کنید.
...
اکنون باید بتوانید **اشکال‌زدایی با DevTools** را توضیح دهید، در نمونه تشخیص دهید، به‌صورت مرحله‌ای پیاده کنید و با ابزار مرورگر بسنجید. گام بعدی **جلسهٔ 48: کارایی CSS و صفحه** است؛ پیش از رفتن، تمرین چالشی را نگه دارید تا در جلسهٔ بعد روی همان خروجی توسعه دهید.
