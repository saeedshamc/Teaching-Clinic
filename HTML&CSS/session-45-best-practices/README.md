...
**جلسهٔ 45 — سازمان‌دهی و خوانایی CSS | ۹۰ دقیقه**

### پیش‌نیازها

- مفاهیم جلسهٔ 44: تایپوگرافی Fluid
...
1. مفهوم «سازمان‌دهی و خوانایی CSS» را با یک مثال و واژگان فارسی/English توضیح دهید.
...
موضوع **سازمان‌دهی و خوانایی CSS** یک جزیرهٔ جدا نیست؛ مستقیماً روی کیفیت، نگهداری و تحویل رابط در چرخهٔ واقعی توسعه اثر می‌گذارد. یادگیری آن به‌صورت تصمیم‌محور باعث می‌شود به‌جای حفظ کردن تگ یا property، بتوانید برای محتوای واقعی راه‌حل درست انتخاب کنید و دلیل انتخابتان را توضیح دهید.

### مفهوم عمیق: از کد تا نتیجه

مرورگر کد را از بالا به پایین parse می‌کند، آن را به ساختارهای داخلی مانند DOM و CSSOM تبدیل می‌کند و سپس style، layout و paint را انجام می‌دهد. هنگام کار با **سازمان‌دهی و خوانایی CSS** همیشه سه پرسش داشته باشید: «معنای این کد چیست؟»، «مرورگر چگونه آن را محاسبه می‌کند؟» و «کاربر با keyboard، موبایل یا صفحه‌خوان چه تجربه‌ای دارد؟». این مدل ذهنی از آزمون‌وخطای تصادفی جلوگیری می‌کند.

### ساختار پیشنهادی فایل‌ها

CSS را به ترتیب `settings/tokens`، پایه، layout، component و utility سازمان دهید. selector کوتاه، نام معنادار، یک مسئولیت برای هر component و کامنت برای «چرایی» خوانایی را بالا می‌برد. فایل خروجی می‌تواند برای production bundle شود.
...
فایل [`examples/01_file_structure.html`](./examples/01_file_structure.html) را مبنا قرار دهید:

1. فایل را در مرورگر باز کنید و بدون دیدن کد، اجزای صفحه را نام ببرید.
2. سورس را بخوانید و هر بخش مرتبط با «سازمان‌دهی و خوانایی CSS» را با یک کامنت مشخص کنید.
...
مثال اصلی را از حافظه در یک فایل تازه بازسازی کنید و سه تصمیم خود دربارهٔ «سازمان‌دهی و خوانایی CSS» را در کامنت‌ها توضیح دهید. سپس یک screenshot یا یادداشت از بررسی DevTools ثبت کنید.
...
اکنون باید بتوانید **سازمان‌دهی و خوانایی CSS** را توضیح دهید، در نمونه تشخیص دهید، به‌صورت مرحله‌ای پیاده کنید و با ابزار مرورگر بسنجید. گام بعدی **جلسهٔ 46: مبانی Sass** است؛ پیش از رفتن، تمرین چالشی را نگه دارید تا در جلسهٔ بعد روی همان خروجی توسعه دهید.
